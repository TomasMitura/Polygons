...
for i, triangle in enumerate(triangles):
    ...
    xy_plane_points = calculate_bounding_box(vertices, step_size_x, step_size_y)
    n += 1
    print('Found bouding box: ' +str(n))
    ...
    for point in xy_plane_points:
        ...
        #print('Found intersection point' +str(m)) 
        if intersection_point is not None:
            is_inside = is_point_inside_triangle(vertex0, vertex1, vertex2, intersection_point, triangle_normal)
            j += 1
            #print('Checked if point is inside' + str(j))
            if is_inside:
                if abs(denominator) > 1e-6:
                    
                    #print('Appended point' +str(k))
                    intersection_points.append(intersection_point)  
            else:
                k += 1
                print('Not inside' +str(k))

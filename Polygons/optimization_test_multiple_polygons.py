...
def insert_sorted(lst, point):
    i = 0
    while i < len(lst) and (lst[i][1] < point[1] or (lst[i][1] == point[1] and lst[i][0] < point[0])):
        i += 1
    lst.insert(i, point)
...
def draw_contours(edges, key, depth_file, repeat, laser_output, x_min, x_max, y_min, y_increment, y_max, base_depth, end_depth, slow_speed, speed_off, base_speed, fab_file):
        ...
        while y_coord <= y_max:
            ...
            for edge in edge_array:
                ...
                if intersection is not None:
                    insert_sorted(intersection_points, intersection)
    ...
    while repeat_counter <= repeat:    
        for i in range(0, len(intersection_points)-1, 2):
            ...
            #laser_output = interp_func(depth) #in case I come back to standardized depth
            if abs(x1-x2)<0.0001:
                pass
            else:    
                if (x2 - x1) < 0.09:
...
with open(output_file, "w") as fab_file:
    draw_contours(edges, key, depth_file, repeat, laser_output, x_min, x_max, y_min, y_increment, y_max, base_depth, end_depth, slow_speed, speed_off, base_speed, fab_file)
